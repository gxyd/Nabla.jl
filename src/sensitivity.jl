...
macro explicit_intercepts(
    ...
    ex = explicit_intercepts(f, type_tuple, isnode, __source__; parse_kwargs(kwargs)...)
    # The result contains all method definitions generated for f
    return esc(ex)
end
...
function boxed_method(
    ...
    # If keyword arguments are provided, add them to the generated call to `Branch`
    if !isempty(kwargs)
        params = Expr(:parameters)
        for (k, _) in kwargs
            push!(params.args, Expr(:kw, k, k))
        end
        insert!(body.args, 2, params)
    end
...
`preprocess` is invoked with `y` and `xs` still boxed.
The default implementation just calls `unbox` on them then calls `preprocess` on the unboxed
values.
If for preprocessing you need the boxed values you should overload
`preprocess(f, y::Node, ȳ, xs...)`.
If you need them unboxed, then overloading `preprocess(f, y, ȳ, xs...)` is fine.
"""
@inline preprocess(f, y, ȳ, xs...) = ()
@inline preprocess(f, y::Node, ȳ, xs...) = preprocess(f, unbox(y), ȳ, map(unbox, xs)...)
